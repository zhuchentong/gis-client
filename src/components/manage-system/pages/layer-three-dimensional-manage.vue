<template>
  <section class="page layer-three-dimensional-manage">
      ...
      <template slot="columns">
        ...
        <el-table-column label=" 操作">
          <template slot-scope="scope">
            <el-button @click="editClick(scope.row)" type="text"
              >修改</el-button
            >
            <el-button @click="deleteLayerClick(scope.row)" type="text"
              >删除</el-button
            >
          </template>
        </el-table-column>
    ...
    <el-dialog
      :title="`${layer.id ? '修改' : '新增'}三维图层配置`"
      :center="true"
      :visible.sync="dialog.modifyLayer"
      :close-on-press-escape="false"
      :close-on-click-modal="false"
      :show-close="false"
    >
      <modify-three-dimensional-layer
        :layer="layer"
        @close="dialog.modifyLayer = false"
        @success="refreshData"
      ></modify-three-dimensional-layer>
    </el-dialog>
  </section>
