  ...
  <section class="component compute-earning">
    <el-form :model="model" label-width="120px" ref="form">
      ...
      <el-form-item label="评估单价" prop="assessmentPrice">
        <el-input v-model="model.assessmentPrice" type="number" :min="0" :max="999999">
          <span slot="append">{{currentUnit.name}}</span>
        </el-input>
      </el-form-item>
      <el-form-item label="基准地价" prop="landPrice">
        <el-input v-model="model.landPrice" type="number" :min="0" :max="999999">
          <span slot="append">{{currentUnit.name}}</span>
        </el-input>
      </el-form-item>
    ...
    <el-form v-if="resultFlag" :model="result" label-width="120px" ref="result">
...
export default class ComputeEarning extends Vue {
  ...
  private compute() {
    ...
    this.result.anticipated = 49332.33
    this.result.assessment = 41002.33
